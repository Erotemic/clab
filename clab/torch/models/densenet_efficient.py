...
"""
From:
    https://raw.githubusercontent.com/gpleiss/efficient_densenet_pytorch/master/models/densenet_efficient.py
"""
...
class _EfficientDensenetBottleneck(nn.Module):
    ...
    def __init__(self, shared_allocation_1, shared_allocation_2, num_input_channels, num_output_channels):
        ...
        self.conv_weight = nn.Parameter(torch.Tensor(
            num_output_channels, num_input_channels, 1, 1))
...
class _DenseBlock(nn.Container):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate, storage_size=1024):
        ...
        self.final_num_features = (num_input_features +
...
class _EfficientDensenetBottleneckFn(Function):
    ...
    def __init__(self, shared_allocation_1, shared_allocation_2,
        ...
        self.efficient_conv = _EfficientConv2d(
    ...
    def forward(self, bn_weight, bn_bias, conv_weight, *inputs):
        ...
        bn_output = self.efficient_batch_norm.forward(
            bn_weight, bn_bias, bn_input)
        relu_output = self.efficient_relu.forward(bn_output)
        conv_output = self.efficient_conv.forward(
            conv_weight, None, relu_output)
    ...
    def backward(self, grad_output):
        ...
        bn_output = self.efficient_batch_norm.forward(
            self.bn_weight, self.bn_bias, cat_output)
        relu_output = self.efficient_relu.forward(bn_output)
...
class _EfficientReLU(object):
    ...
    def forward(self, input):
        ...
        backend.Threshold_updateOutput(
            backend.library_state, input, output, 0, 0, True)
        return output
